...
async function getAllImageAndVideoProfileFromStorage() {
    ...
    const refVideo = storage.child('profile/video');
    const listResultVideo = await refVideo.listAll();
    for (const item of listResultVideo.items) {
        const source = await item.getDownloadURL();
        videoUrls.push({ source: source });
    }
    ...
    if (imageUrls.length<3){
        ...
        for (let i = 0; i < imageUrls.length; i++) {
            if ((i % 10 == 2 || i % 10 == 5) && y < videoUrls.length){
                allUrls.push(videoUrls[y]);
        ...
        if(y <videoUrls.length){
            allUrls.push(...videoUrls.slice(y));
        }
    }
    else if (videoUrls.length==0){
        allUrls.push(...imageUrls);
